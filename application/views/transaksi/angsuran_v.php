...
<form action="" method="post" autocomplete="off" class="pull-right" id="search_report">
        ...
        <button type="button" id="print" class="btn btn-xs btn-primary hide" style="height:27px;"><i class="glyphicon glyphicon-print"></i> Print</button>               
    </form>
	<table id="example88" class="table table-bordered table-striped">
		<thead><tr>
			<th style="text-align:center; width:4%">No.</th>
			<th style="text-align:center; width:15%">No.Tagihan</th>
                       
            <th style="text-align:center; width:18%">Transaksi</th>	
            <th style="text-align:center; width:20%">Member</th>		
            <th style="text-align:center; width:20%">Angsuran</th>		
			...
			<?php 
				...
				if(!empty($tagihan)){		
					foreach($tagihan as $t){						
						...
						$tgl_tempo = '';
						$tgl_tempo = date('Y-m-d', strtotime($t['tgl_tempo']));
						if($tgl_tempo < $tgl){						
							$_span = '<span class="label label-danger">Tunggakan</span>';
						}						
						$_action .= '<a href="'.site_url('tagihan/detail/'.$info).'"><button style="margin-top : 5px; width:69px;" title="View" class="btn btn-xs btn-info"><i class="fa fa-eye"></i> View</button></a>';
						echo '<tr>';
						echo '<td align="center">'.$i++.'.</td>';						
						echo '<td>'.$t['kode_payment'].'<br/>'.$_span.'</td>';
						echo '<td>No.Transaksi : #'.$t['id_transaksi'].'<br/>Tanggal : '.date("d M Y", strtotime($t['create_at'])).'<br/>Total : '.number_format($t['ttl_all'],2,',','.').'</td>';
						echo '<td>'.$t['nama_member'].' - '.$t['phone_member'].'<br/> Email : '.$t['email_member'].'</td>';	
						echo '<td>Angsuran ke : '.$t['angs_ke'].' dari '.$t['tempo'].'<br/>Angsuran : '.number_format($t['angs'],2,',','.').'<br/> Tgl.Tempo : '.date("d M Y", strtotime($t['tgl_tempo'])).'</td>';
