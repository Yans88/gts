            ...
            <div class="modal-content">
              ...
              <div class="modal-body" style="padding-bottom:2px;">
				...
				<div class="form-group">
                  <label>Nama Brand</label><span class="label label-danger pull-right provinsi_error"></span>
                  <input type="text" class="form-control" name="nama_brand" id="nama_brand" value="" placeholder="Nama Brand" autocomplete="off" />
                  <input type="hidden" value="" name="id_brand" id="id_brand">
                </div>
                
				<div class="form-group">
                  <label>OcrCode</label><span class="label label-danger pull-right ocrcode_b_error"></span>
                  <input type="text" class="form-control" name="ocrcode_b" id="ocrcode_b" value="" placeholder="OcrCode" autocomplete="off" />
...
<div class='alert alert-info alert-dismissable' id="success-alert">
	...
	<table id="example88" class="table table-bordered table-striped">
		<thead><tr>
			...
			<th style="text-align:center; width:10%">ID Brand</th>			
			<th style="text-align:center; width:30%">Nama Brand</th>
			<th style="text-align:center; width:30%">OcrCode</th>
		...
		<tbody>
			<?php 
				...
				if(!empty($brand)){		
					foreach($brand as $c){	
						...
						$info = $c['id_brand'].'Þ'.$c['nama_brand'].'Þ'.$c['ocrcode_b'];
						echo '<tr>';
						echo '<td align="center">'.$i++.'.</td>';
						echo '<td>'.$c['id_brand'].'</td>';
						echo '<td>'.ucwords($c['nama_brand']).'</td>';
						echo '<td>'.ucwords($c['ocrcode_b']).'</td>';
			...
			<button title="Delete" id="'.$c['id_brand'].'" class="btn btn-xs btn-danger del_category"><i class="fa fa-trash-o"></i> Delete</button>		
						</td>';
						echo '</tr>';
					}
				}
...
$('.yes_del').click(function(){
	var id = $('#del_id').val();
	var url = '<?php echo site_url('brand/del');?>';
	$.ajax({
		data : {id : id},
		url : url,
		type : "POST",
...
$('.yes_save').click(function(){
	var provinsi = $('#nama_brand').val();
	$('.provinsi_error').text('');
	if(provinsi <= 0 || provinsi == '') {
		$('.provinsi_error').text('Nama brand harus diisi');
		return false;
	}
	
	var url = '<?php echo site_url('brand/simpan');?>';
